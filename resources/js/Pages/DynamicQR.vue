<template>
  <div :class="isDark ? 'dark bg-zinc-950 text-zinc-100' : 'bg-zinc-100/80 text-zinc-900'" class="h-screen w-screen overflow-hidden flex flex-col font-sans antialiased selection:bg-zinc-200 selection:text-black">
    ...
    <header :class="isDark ? 'bg-zinc-900/90 border-zinc-800' : 'bg-white border-zinc-200'" class="h-14 border-b flex-shrink-0 flex items-center justify-between px-4 sm:px-6 backdrop-blur z-30">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2">
          <span class="text-sm font-semibold tracking-tight truncate max-w-[200px] sm:max-w-xs">{{ merchant.merchantName || 'Genie Business Merchant' }}</span>
    ...
    <!-- Main Workspace (Zero-Scroll 100vh Flex Container) -->
    <main class="flex-1 p-4 sm:p-6 overflow-hidden max-w-7xl w-full mx-auto flex flex-col">
      
      <!-- Compact Toast Notification (if active) -->
      ...
      <!-- Two-Column Viewport Fit Grid -->
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1 min-h-0 items-stretch">
        
        <!-- LEFT COLUMN: Payment Register & Touch Keypad (6 cols) -->
        <div :class="isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200/90 shadow-sm'" class="lg:col-span-6 border rounded-2xl p-5 flex flex-col justify-between overflow-hidden">
          
          <!-- Top Amount Display Box -->
          <div>
            <div class="flex items-center justify-between pb-2 mb-3 border-b" :class="isDark ? 'border-zinc-800' : 'border-zinc-100'">
              <span class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">Cashier Payment Register</span>
            ...
            <!-- Amount Input / Readout -->
            <div :class="isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'" class="border rounded-xl py-3 px-4 text-center">
              ...
              <div class="flex items-baseline justify-center gap-2 mt-0.5">
                <span class="text-base font-mono font-medium text-zinc-400">Rs.</span>
                <input 
                  type="number" 
                  step="0.01" 
                  min="1"
                  v-model="transaction.amount" 
                  class="bg-transparent text-3xl sm:text-4xl font-bold font-mono text-center focus:outline-none w-44 max-w-full"
            ...
            <div class="mt-2.5 flex items-center justify-between gap-1.5">
              <div class="flex flex-wrap items-center gap-1.5">
                <button 
                  ...
                  class="px-2.5 py-1 rounded-lg border text-xs font-mono font-medium transition active:scale-95">
                  +{{ amt }}
                </button>
              </div>
              <button 
                ...
                class="px-1.5 py-0.5 text-xs font-mono underline">
                Reset (10)
              </button>
            </div>
          </div>

          <!-- Middle: Touch Keypad Grid -->
          <div class="my-3">
            <div class="grid grid-cols-3 gap-1.5">
              <button 
                ...
                class="py-2.5 rounded-xl border font-mono font-semibold text-base transition active:scale-95 flex items-center justify-center">
                {{ key }}
              </button>
            </div>
          </div>

          <!-- Bottom: Invoice Reference Row -->
          <div class="pt-2.5 border-t" :class="isDark ? 'border-zinc-800' : 'border-zinc-100'">
            ...
            <input 
              ...
              class="w-full border rounded-lg px-3 py-1.5 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-zinc-500 transition"
            />
          </div>

        </div>

        <!-- RIGHT COLUMN: Customer Standee Display & Live QR (6 cols) -->
        <div :class="isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200/90 shadow-md'" class="lg:col-span-6 border rounded-2xl p-5 sm:p-6 flex flex-col justify-between items-center text-center overflow-hidden">
          ...
          <div :class="isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'" class="w-full border rounded-xl py-2.5 px-4 my-1">
            ...
            <div v-if="transaction.referenceLabel" class="text-[11px] text-zinc-500 font-mono mt-0.5">
          ...
          <div class="w-full text-[10px] font-mono text-zinc-400 flex items-center justify-center gap-2 my-1">
            <span>VISA</span>
            <span>•</span>
          ...
          <!-- Action Toolbar -->
          <div class="w-full grid grid-cols-3 gap-2 pt-2 border-t" :class="isDark ? 'border-zinc-800' : 'border-zinc-100'">
            <button 
              type="button" 
            ...
            <button 
              ...
              :class="isDark 
                ? 'bg-zinc-950 hover:bg-zinc-800 text-zinc-200 border-zinc-800' 
                : 'bg-zinc-50 hover:bg-zinc-100 text-zinc-800 border-zinc-200 shadow-sm'"
              class="border text-[11px] font-mono py-2 rounded-lg transition flex items-center justify-center gap-1">
              Inspector
            </button>
          </div>
    ...
    <!-- Inspector Modal (So it doesn't crowd main screen) -->
    <div v-if="showInspectorModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div :class="isDark ? 'bg-zinc-900 border-zinc-800 text-zinc-100' : 'bg-white border-zinc-200 text-zinc-900'" class="border rounded-2xl max-w-xl w-full p-6 shadow-2xl max-h-[85vh] overflow-y-auto">
        
        <div class="flex items-center justify-between pb-3 mb-3 border-b" :class="isDark ? 'border-zinc-800' : 'border-zinc-100'">
          <div>
            <h3 class="text-sm font-semibold">EMVCo String & TLV Inspector</h3>
            <span class="text-xs text-zinc-400 font-mono">{{ qrPayload.length }} characters • CRC: {{ qrData.crc }}</span>
          </div>
          <button @click="showInspectorModal = false" class="text-zinc-400 hover:text-zinc-600 text-lg">✕</button>
        </div>

        <div class="space-y-4">
          <div>
            <div class="flex justify-between items-center mb-1">
              <span class="text-xs text-zinc-500 font-mono">Raw QR String</span>
              <button @click="copyPayload" class="text-xs font-mono underline text-zinc-400 hover:text-white">
                {{ copied ? 'Copied!' : 'Copy' }}
              </button>
            </div>
            <div :class="isDark ? 'bg-zinc-950 border-zinc-800 text-zinc-300' : 'bg-zinc-50 border-zinc-200 text-zinc-800'" class="border rounded-lg p-3 text-[11px] font-mono break-all select-all leading-relaxed">
              {{ qrPayload }}
            </div>
          </div>

          <div>
            <span class="text-xs text-zinc-500 font-mono block mb-2">Decoded Tag Structure ({{ decodedTags.length }} Tags)</span>
            <div class="space-y-1.5 max-h-60 overflow-y-auto font-mono text-xs pr-1">
              <div 
          ...
          <div class="pt-3 border-t flex justify-end" :class="isDark ? 'border-zinc-800' : 'border-zinc-100'">
            <button 
              type="button" 
