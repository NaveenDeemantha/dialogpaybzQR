...
class EmvQrService
{
    ...
    public function generateDynamicQrPayload(array $params): array
    {
        ...
        $pointOfInitiation = $params['pointOfInitiationMethod'] ?? '12';
        $tags[] = $this->formatTlv('01', $pointOfInitiation);

        // 02: Visa Active (Length: 16)
        if (!empty($params['visaPan'])) {
            $tags[] = $this->formatTlv('02', substr($params['visaPan'], 0, 16));
        }

        // 03: Visa Reserved (Length: 16)
        if (!empty($params['visaPan'])) {
            $tags[] = $this->formatTlv('03', substr($params['visaPan'], 0, 16));
        } elseif (!empty($params['visaReserved'])) {
            $tags[] = $this->formatTlv('03', substr($params['visaReserved'], 0, 16));
        }

        // 04: Mastercard Active (Length: 16)
        if (!empty($params['mastercardPan'])) {
            $tags[] = $this->formatTlv('04', substr($params['mastercardPan'], 0, 16));
        }

        // 05: Mastercard Reserved (Length: 16)
        if (!empty($params['mastercardPan'])) {
            $tags[] = $this->formatTlv('05', substr($params['mastercardPan'], 0, 16));
        } elseif (!empty($params['mastercardReserved'])) {
            $tags[] = $this->formatTlv('05', substr($params['mastercardReserved'], 0, 16));
        }

        // 15: UnionPay Active (Optional, Length: 31)
        if (!empty($params['unionpayPan'])) {
            $tags[] = $this->formatTlv('15', substr($params['unionpayPan'], 0, 31));
        }

        // 16: UnionPay Reserved (Optional, Length: 31)
        if (!empty($params['unionpayPan'])) {
            $tags[] = $this->formatTlv('16', substr($params['unionpayPan'], 0, 31));
        } elseif (!empty($params['unionpayReserved'])) {
            $tags[] = $this->formatTlv('16', substr($params['unionpayReserved'], 0, 31));
        }

        // 26: Globally unique identifier data object / Merchant Account Information (Length: 32)
        if (!empty($params['merchantGuidAcquirerId'])) {
            $tags[] = $this->formatTlv('26', substr($params['merchantGuidAcquirerId'], 0, 32));
        } elseif (!empty($params['merchantAcquiringBankId'])) {
            $tags[] = $this->formatTlv('26', substr($params['merchantAcquiringBankId'], 0, 32));
        }

        // 52: Merchant Category Code (Mandatory, 4 digits e.g. '5300')
        $mcc = !empty($params['merchantMccCode']) ? str_pad($params['merchantMccCode'], 4, '0', STR_PAD_LEFT) : '5300';
        ...
        $tags[] = $this->formatTlv('53', str_pad(substr($currency, 0, 3), 3, '0', STR_PAD_LEFT));

        // 54: Transaction Amount (Mandatory for Dynamic QR, formatted up to 13 chars e.g. '1500.00' or '250')
        if (isset($params['amount']) && $params['amount'] !== '' && $params['amount'] !== null) {
            $amountVal = number_format((float)$params['amount'], 2, '.', '');
            $tags[] = $this->formatTlv('54', $amountVal);
        }

        // 58: Country Code (Mandatory: 'LK')
        $countryCode = !empty($params['merchantCountryCode']) ? strtoupper(substr($params['merchantCountryCode'], 0, 2)) : 'LK';
        $tags[] = $this->formatTlv('58', $countryCode);

        // 59: Merchant Name (Mandatory, max 25 chars)
        $merchantName = !empty($params['merchantName']) ? substr($params['merchantName'], 0, 25) : 'Genie Merchant';
        $tags[] = $this->formatTlv('59', $merchantName);

        // 60: Merchant City (Mandatory, max 15 chars)
        $merchantCity = !empty($params['merchantCity']) ? substr($params['merchantCity'], 0, 15) : 'Colombo';
        $tags[] = $this->formatTlv('60', $merchantCity);

        // 62: Additional Data Field Template (Mandatory)
        // Sub-tag 05: Reference Label (Invoice / Order / Reference Number)
        $reference = !empty($params['referenceLabel']) ? $params['referenceLabel'] : '00000000000';
        $referenceTlv = $this->formatTlv('05', substr($reference, 0, 25));
        $tags[] = $this->formatTlv('62', $referenceTlv);
